...
# SEED WEBSITES
	# file = File.join(Rails.root, 'app', 'files', 'germany', 'TopSites-(Shopping)--(276)--(Month_2017_10_1).xlsx')
 #    xlsx = Roo::Spreadsheet.open(file)
 #    xlsx = Roo::Excelx.new(file)
 #    xlsx.sheet(1).column(1).each do |site|
 #    	unless site == 'Domain' || Website.where(url: site.downcase).take
 #    		Website.create!(url: site.downcase, valid_for_yandex: true, category: 0, status: 1, country_id: Country.where(name: "Germany").take.id)
 #    		if File.join(Rails.root, 'app', 'files', 'germany', "GeographyExtended-(#{site.downcase})--(999)--(Month_2017_10_1).xlsx")
 #    			file = File.join(Rails.root, 'app', 'files', 'germany', "GeographyExtended-(#{site.downcase})--(999)--(Month_2017_10_1).xlsx")
 #    			p file
 #    			unless file == ''
	# 			    xlsx = Roo::Spreadsheet.open(file)
	# 			    xlsx = Roo::Excelx.new(file)
...
mother_country = Country.where(name: "China").take
