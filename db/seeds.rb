...
# SEED WEBSITES
	file = File.join(Rails.root, 'app', 'files', 'usa', 'TopSites-(Shopping)--(840)--(Month_2017_10_1).xlsx') #change 
    xlsx = Roo::Spreadsheet.open(file)
    xlsx = Roo::Excelx.new(file)
    xlsx.sheet(1).column(1).each do |site|
    	unless site == 'Domain' || Website.where(url: site.downcase).take
    		#change 
    		Website.create!(url: site.downcase, valid_for_yandex: true, category: 0, status: 1, country_id: Country.where(name: "United States").take.id)
    		#change 
    		if File.join(Rails.root, 'app', 'files', 'usa', "GeographyExtended-(#{site.downcase})--(999)--(Month_2017_10_1).xlsx")
    			#change 
    			file = File.join(Rails.root, 'app', 'files', 'usa', "GeographyExtended-(#{site.downcase})--(999)--(Month_2017_10_1).xlsx")
    			p file
    			unless file == ''
				    xlsx = Roo::Spreadsheet.open(file)
				    xlsx = Roo::Excelx.new(file)
...
# mother_country = Country.where(name: "Vietnam").take
